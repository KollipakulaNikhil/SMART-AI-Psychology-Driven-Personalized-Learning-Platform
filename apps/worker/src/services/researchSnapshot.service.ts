...
import { userAgent } from "@smart-ai/core/services/research/scholar.service";
import { uploadArtifact, deleteArtifactsByPrefix } from "@smart-ai/core/storage/blob";
import type { ResearchPaper, ResearchPatent } from "@smart-ai/core/models/ResearchProject";
...
async function saveSnapshot(projectId: string, fileName: string, png: Buffer): Promise<string> {
  const { url } = await uploadArtifact("research", projectId, fileName, png, { contentType: "image/png" });
  return url;
}
...
/** Removes a project's snapshots (on delete). */
export async function removeSnapshots(projectId: string): Promise<void> {
  await deleteArtifactsByPrefix("research", projectId).catch(() => undefined);
}

...
import type { SlideBoard, SlideContent } from "@smart-ai/core/models/Presentation";
import { fetchArtifactBuffer } from "@smart-ai/core/storage/blob";
 ...
 *
 * Frames are written to local scratch disk (not Blob) — they're consumed
 * immediately by ffmpeg in this same job and discarded afterward; only the
 * final rendered video is a durable artifact.
 */
...
/**
 * A framed photo on the right of the board, embedded as base64 from a
 * pre-fetched buffer (the slide image lives in Blob storage now, not on
 * local disk — see `renderBoardDeck`'s pre-fetch pass).
 */
function imageSvg(imageBuffer: Buffer | undefined, imageCredit: string | undefined): string {
  ...
  ${imageCredit ? `<text x="${x + w}" y="${y + h + 32}" text-anchor="end" font-family="${SANS}" font-size="18" fill="${C.faint}">${esc(imageCredit)}</text>` : ""}`;
}
...
function renderSlideState(
  ...
  const hasImage = !hasDiagram && Boolean(imageBuffer);
  ...
  const rightSide = hasDiagram ? diagramSvg(slide.board, nodesToShow) : imageSvg(imageBuffer, slide.imageCredit);
...
async function renderCover(dir: string, title: string, topic: string, learnerLine: string): Promise<string> {
...
export async function renderBoardDeck(
  ...
  const dir = scratchDir(presentationId, "slides", "board");
  const total = slides.length + 2;

  const coverPath = await renderCover(dir, title, topic, learnerLine);

  // Pre-fetch every slide image once (only needed for slides without a diagram).
  const imageBuffers = await Promise.all(
    slides.map((slide) =>
      slide.imagePath && !(slide.board?.diagram && slide.board.diagram.type !== "none")
        ? fetchArtifactBuffer(slide.imagePath).catch(() => undefined)
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    for (let state = 0; state < states; state++) {
      ...
      await renderFrame(renderSlideState(slide, imageBuffers[i], state, pageLabel), outPath);
      frames.push(outPath);
    }
...
export function slidesHaveBoard(slides: SlideContent[]): boolean {
  return slides.some((slide) => (slide.board?.keyTerms?.length ?? 0) > 0 || Boolean(slide.board?.diagram));
}

...
/**
 * Builds the downloadable 16:9 deck and uploads it to Blob storage.
 *
...
export async function buildPptx(input: PptBuildInput): Promise<string> {
  ...
  async function addConceptPage(slideContent: SlideContent, index: number): Promise<void> {
    const slide = pptx.addSlide({ masterName: "SMART_CONTENT" });
    const hasImage = Boolean(slideContent.imagePath);
    ...
    if (hasImage) {
      const imageBuffer = await fetchArtifactBuffer(slideContent.imagePath as string);
  ...
  for (const [index, slideContent] of slides.entries()) {
    await addConceptPage(slideContent, index);
  ...
  const uploaded = await uploadArtifact("ppt", presentationId, "presentation.pptx", buffer, {

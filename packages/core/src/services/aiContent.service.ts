...
import { callOpenAI, callOpenAIText, generateLessonContentOpenAI } from "./openai.service";
...
const PROVIDER_RUNNERS: Record<Provider, typeof generateLessonContentGemini> = {
  openai: generateLessonContentOpenAI,
 ...
 * and is allowed to fail; Gemini/OpenAI have no small tier configured here so
 * they reuse the normal caller.
 */
...
/** OpenAI first (explicitly configured as the primary provider), then Gemini, then Groq — only providers with a configured key are tried. */

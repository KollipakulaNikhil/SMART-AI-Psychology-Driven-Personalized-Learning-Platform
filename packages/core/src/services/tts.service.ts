...
export interface NarrationResult {
  /** Raw mp3 bytes — the caller uploads this to Blob storage. */
  audioBuffer: Buffer;
  /**
...
async function synthesizeWithEdge(
  ...
  return withRetry(
    async () => {
      ...
      return { audioBuffer: Buffer.concat(chunks), wordTimings: parseWordTimings(metaChunks) };
    },
...
export async function synthesizeNarration(request: NarrationRequest): Promise<NarrationResult> {
  const { slideIndex, text, traits, speed, previousText, nextText } = request;
  ...
  if (!env.ELEVENLABS_API_KEY || !languageDefinition(language).preferElevenLabs) {
    return synthesizeWithEdge(spokenText, traits.tone, speed, language);
  }
  ...
  const requestSpeech = async (includeSpeed: boolean): Promise<Buffer> => {
    ...
    const { data } = await axios.post<ArrayBuffer>(`${ELEVENLABS_TTS_URL}/${voiceId}`, body, {
      headers: {
  ...
  return { audioBuffer: audio, wordTimings: [] };
}

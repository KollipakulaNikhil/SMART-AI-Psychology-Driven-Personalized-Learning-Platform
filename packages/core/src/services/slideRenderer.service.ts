...
async function renderSvg(svgBody: string): Promise<Buffer> {
...
export async function renderCoverSlide(title: string, topic: string, learnerLine: string): Promise<Buffer> {
...
export async function renderContentSlide(
  ...
  let imageSvg = "";
  if (hasImage) {
    const imageBuffer = await fetchArtifactBuffer(slide.imagePath as string);
...
export async function renderNotesSlide(
  ...
  return renderSvg(`${head.svg}${left}${right}${footer(pageLabel)}`);
}
...
export async function renderPracticeSlide(
  ...
  return renderSvg(`${head.svg}${body}${footer(pageLabel)}`);
}
...
export async function renderEndSlide(title: string, summary: string, pageLabel: string): Promise<Buffer> {
...
export interface RenderedDeck {
  /** Every page of the handout, in order: cover, per-topic pages, practice, quiz, answers, closing — as Blob URLs. */
  allPaths: string[];
  coverPath: string;
  /**
...
/**
 * Renders the full handout as 1920×1080 PNGs, each uploaded to Blob storage —
...
export async function renderDeck(input: RenderDeckInput): Promise<RenderedDeck> {
  ...
  interface PageJob {
    slideIndex?: number;
    render: (pageLabel: string) => Promise<Buffer>;
  }

  const jobs: PageJob[] = [{ render: () => renderCoverSlide(title, topic, learnerLine) }];

  slides.forEach((slide, index) => {
    jobs.push({
      slideIndex: index,
      render: (label) => renderContentSlide(slide, label, imageEmphasis),
    });
    const notes = slide.studyNotes;
    if (notes?.explanation) {
      jobs.push({ render: (label) => renderNotesSlide(slide.title, index, notes, label) });
    }
    if (notes?.example || notes?.practice) {
      jobs.push({ render: (label) => renderExampleSlide(slide.title, index, notes, label) });
    }
  });
